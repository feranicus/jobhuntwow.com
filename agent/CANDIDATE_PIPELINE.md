# Candidate pipeline — Hermes talks, the stack applies

Hermes is kept for what it is genuinely good at: **talking to the candidate and writing prose**.
The Playwright/Stagehand stack does the clicking. One universal profile feeds every ATS.

```
  CANDIDATE (Telegram / portal)
        |  Hermes interviews once  (asks only what's missing)
        v
   userdata/candidate.md          <-- UNIVERSAL, ATS-agnostic single source of truth
        |
        +--> per JOB: Hermes + DO models write tailored resume.pdf + cover_letter.pdf
        |
        v
...
## 1. Hermes's job (candidate-facing)
