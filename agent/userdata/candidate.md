...
# MEASURED FROM THE OPERATOR'S OWN RECORDING, 2026-08-17. On Workday AND Greenhouse the box above is
# REJECTED: the Accenture recording shows `+49 157 8551545`, `+49 1578551545` and `1578551545` each
# producing `Error Phone Number`, and only `15785541545` accepted. The country code belongs in the
# country selector, never in the text. Adapters call `greenhouse.phone_national()` / the same rule in
# workday, so `phone` above stays canonical for documents and CVs and `phone_national` is what a form
# receives. Do not "fix" the value above to match — a CV needs the international form.
- phone_national: "15785541545"
- phone_country: "Germany"
- phone_type: Mobile
- country: Germany
- address_line: Hermann-J.-Bach-Weg 16
- city: Friedberg
# Greenhouse's Location (City) box is an ASYNC react-select: he typed "Friedberg 61169", got NO
# options, and retyped "Friedberg" -> "Friedberg, Hesse, Germany". Query with the city ALONE.
- city_query: Friedberg
- state_region: Hessen
- postal_code: "61169"
...
## documents
- resume_default: "C:\\Users\\feran\\Downloads\\Resumes and positions\\Evgeny Vainshtein - CV 2026.pdf"
- resume_master_html: "agent/templates/resume.html"  # CSS source; facts in resume_data.json
- cover_master_html: "agent/templates/cover_letter.html"
- resume_tailored: ""
- cover_letter_tailored: ""
- note: "Tailored files are regenerated per job description; paths written back here."

## screening_defaults
# DECLARED BY THE CANDIDATE IN HIS OWN RECORDING, okx_greenhouse.py (2026-08-17). These are the
# answers HE chose on a real EEOC block, so they are a written declaration, not a guess -- which is
...
- visa sponsorship: No
- how_did_you_hear: "LinkedIn"             # if LinkedIn is not offered, take the FIRST real option - it does not matter
...
# The questions NTT (Phenom) asks on step 2. Same wording recurs across most ATS, so answering
# them here stops the model inventing a value 14 times in a row.
- worked_for_company_before: "No"          # ALWAYS No - any "have you worked here before, directly or indirectly / via an agency" question
- current_employee_of_company: "No"         # ALWAYS No, on every form
- willing_to_accept_travel_assignment: "Yes"  # ALWAYS Yes
- willing_to_relocate: "No"                 # ALWAYS No (Frankfurt/DACH commute + remote)
- bound_by_noncompete_or_confidentiality: "No"  # ALWAYS No
- available_start_date: "2026-08-20"       # ALWAYS one month from the application date
- notice_period: "1 month"
- requires_visa_sponsorship: "No"          # EU work authorisation, no sponsorship needed
- salary_expectation_eur: "150000"          # EUR gross p.a.; state the number when asked
- IF AN OPTION IS NOT LISTED: choose the closest available option; never leave a required
  screening question empty and never invent an employer, date or figure.
