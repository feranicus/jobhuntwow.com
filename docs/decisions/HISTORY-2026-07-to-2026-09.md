# JobHuntWOW app — decision history, 2026-07 to 2026-09

This file is the EVIDENCE behind the rules in `CLAUDE.md`. Every incident, root cause, measured
number and fixture that earned a rule is recorded here verbatim, in the order it happened.

**Why it was split out (2026-09-18):** `CLAUDE.md` is re-injected into the model's context on EVERY
turn. It had reached **219 KB / 2,719 lines / 126 sections ≈ 55,000 tokens per turn** — spent before
the operator typed a word, which is what makes a long session run out of room and start forgetting.
Nothing was deleted: 110 narrative sections moved here unchanged, and `CLAUDE.md` kept the rules.
The sibling project (`Linkedin Scraper`) hit the identical wall at 646 KB and was split the same way;
`tests/test_claude_md_size.py` now FAILS THE BUILD if this repo drifts back over budget.

**How to use it:** when a rule in CLAUDE.md matters and you need the story — what broke, what was
measured, which mutation proved the guard — `grep` this file for a distinctive phrase from the rule.

**How to add to it:** a new incident is appended HERE in the same voice and detail. It earns AT MOST
ONE LINE in CLAUDE.md, and only if it is a rule that must be obeyed on every turn.
